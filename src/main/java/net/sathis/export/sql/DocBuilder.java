...
public class DocBuilder {
	...
	public void execute() throws Exception {
		...
		for (Entity entity : importer.getConfig().document.entities) {
			String entityQuery = entity.allAttributes.get("query");
			importer.getWriter().setPrimaryKey(entity.pk);
			importer.getWriter().initCollection(entity.name);
			...
			while (rs.next()) {
				...
				entityList.add(getFields(processor.toMap(rs), rs, entity, null, null));
				i++;
			}
			importer.getWriter().writeToNoSQL(entityList);
		}
